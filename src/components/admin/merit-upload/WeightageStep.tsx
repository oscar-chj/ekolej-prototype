...
import { Box, Typography, Card, CardContent, Chip, Paper, TextField, Alert, Button } from "@mui/material";
...
export function WeightageStep({
    ...
    return (
        <Box>
            <Typography variant="h6" gutterBottom>Set Merit Point Weightages</Typography>
            ...
            <Paper sx={{ p: 3 }}>
                <Typography variant="subtitle1" gutterBottom>Merit Configuration</Typography>
                ...
                <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
                    ...
                    <Alert severity="info">
                        Participants will receive {meritWeightage.participantPoints} points, while organizers will receive {meritWeightage.organizerPoints} points. Maximum threshold is set to {meritWeightage.maxPointsThreshold} points.
                    </Alert>
                </Box>

                <Box sx={{ mt: 3, display: "flex", justifyContent: "space-between" }}>
                    <Button startIcon={<ArrowBack />} onClick={onBack}>Back to Event Selection</Button>
                    <Button variant="contained" onClick={onNext} disabled={isProcessing}>
                        {isProcessing ? "Loading Participants..." : "Next: Review Participants"}
                    </Button>
                </Box>
            </Paper>
        </Box>
